 ...
 * lib/admin/auth.ts's requireAdmin(), which every caller of this module must
 * have already passed), since the session-scoped RPCs (get_usage_total,
 * get_provider_cost_total) are intentionally hardcoded to `auth.uid()` and
 * can't target another user at all.
 */
