...
const ASSIGNABLE_PLANS: PlanId[] = ["starter", "pro", "development"];
const LABELS: Record<PlanId, string> = { starter: "Starter", pro: "Pro", development: "Development" };
...
export function SetPlanControl({ userId, currentPlanId }: { userId: string; currentPlanId: PlanId }) {
  ...
  async function setPlan(planId: string) {
    ...
    try {
      ...
      toast.success(`Plan set to ${LABELS[planId as PlanId]}`);
      router.refresh();
    } catch (err) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <p className="w-full text-xs text-muted-foreground">
        Setting to Starter or Pro resets this account&apos;s plan-change timestamp (Pro&apos;s monthly usage
        period starts fresh from now). Setting to Development removes all product limits.
      </p>
    </div>
  );
}
